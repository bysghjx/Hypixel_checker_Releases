...
public class HypixelSkyWarsQuery implements Callable<HypixelSkyWarsInfo> {
    public final String input_name;
    private final Acceptable<String> onToast;

    public HypixelSkyWarsQuery(String input_name, Acceptable<String> onToast) {
        this.input_name = input_name;
        this.onToast = onToast;
    }

    @Override
    public HypixelSkyWarsInfo call() {
        Looper.prepare();
        ...
        HypixelSkyWarsInfo si = new HypixelSkyWarsInfo();
        int code = HypixelUtils.getSkyWars(uuid, si);
        if (code == -1) {
